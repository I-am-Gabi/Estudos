(function () {
	...
	function increaseInnerNavigation(slideNumber) {
		...
		// 	return -1;
		// }
		var selector = getSlideHash(slideNumber) + ' .inner li:not(.active)';
		selector += ', ' + getSlideHash(slideNumber) + ' .inner > div:not(.active)';

		var nextNodes = document.querySelectorAll(selector);
		console.log(nextNodes);

		if (nextNodes.length > 0) {
			// look for the min order values
			var node, orderVal, min = null;
			for (var i=0; i < nextNodes.length; ++i) {
				orderVal = nextNodes[i].getAttribute("data-order");
				if (!min || (orderVal && orderVal < min)) {
					min = orderVal;
				}
			}
			if (min) {
				// select the min order nodes
				for (var i=0; i < nextNodes.length; ++i) {
					node = nextNodes[i];
					orderVal = node.getAttribute("data-order");
					if (orderVal && orderVal == min) {
						min = orderVal;
						var j = 0;
						do {
							node.classList.add('active');
						}
						while (++j < nextNodes.length && (node = nextNodes[j]) && node.classList.contains('nostep'));
					}
				}
			}
			else {
				...
				while (++i < nextNodes.length && (node = nextNodes[i]) && node.classList.contains('nostep'));
			}
			return 1;
		} else {
			return -1;
